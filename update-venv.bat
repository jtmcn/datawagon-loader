...
python -c "import sys; print('OK' if sys.version_info >= (3, 11) else 'FAIL'); print(f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}')" > "%TEMP%\dw_pyver.txt" 2>nul
if errorlevel 1 (
...
if not "%PY_STATUS%"=="OK" (
    for /f "skip=1 tokens=*" %%V in ('type "%TEMP%\dw_pyver.txt"') do (
        echo [ERROR] Python 3.11+ required ^(found: Python %%V^)
    )
    del "%TEMP%\dw_pyver.txt" 2>nul
    exit /b 1
)
